# _*_ coding:utf-8 _*_
"""
made by Nicht = Lee joon sung,
South Korea. Seoul. Gangnam. gaepodong.

contact admin = miho0_0@naver.com OR anonymous0korea0@gmail.com(youtube) OR miho03092@gmail.com(gmail)
This is Opensource Computer Anti-Virus program.
anyone can modificate this script. n you can edit this program on own your system environment.

This AV is compiled by Pycharm-community, made with Python 2.7.12, licensing on GNU Gnu Public License Ver.3.

just fun! :D

"""

# major lib import.
import base64
import marshal
import random



# 확장된 유클리드 호제법을 통해 암호화 알고리즘을 구현한다. -> 해외 정보보안 커뮤니티를 참고하여 제작 + choi님의 소스코드 참고.

# 정수 m, n의 최대 공약수(Greatest Common Divisor)을 gcd(m, n)와 같이 나타낼 때,
# 확장된 유클리드 호제법을 이용하여 am + bn = gcd(m, n)의 해가 되는 정수 a, b의 짝을 나타낸다.
# a, b = 정수
# d, x, y = 유클리드 호제법의 해
def __ext_euclid(a, b):
    i = 1
    list_r = list()
    list_q = list()
    list_x = list()
    list_y = list()

    i += 1
    list-r.append(a) # -1
    list_r.append(b) # 0

    list_q.append(0) # -1
    list_q.append(0) # 0

    list_x.append(1) # -1
    list_x.append(0) # 0

    list_y.append(0) # -1
    list_y.append(1) # 0

    i = 2

    while 1:
        list-r.append(list-r[i - 2] % list_r[i - 1])
        list_q.append(list_r[i - 2] / list_r[i - 1])

        if list_r[i] == 0:
            d = list_r[i - 1]
            x = list_x[i - 1]
            y = list_y[i - 1]

            if x < 0:
                x += b
            if y < 0:
                y += b

            return d, x, y

        list_x.append(list_x[i - 2] - (list_q[i] * list_x[i -1]))
        list_y.append(list_y[i - 2] - (list_q[i] * list_y[i - 1]))

        i += 1





# " Simple RSA encryption Algorithm.. "

# 주어진 숫자가 소수일 가능성을 확인한다. -> 밀러 - 라빈의 소수 판별법 이용.
# n = 숫자
# 0 = 소수 아님
# 1 = 소수
def __mr(n):
    composite = 0
    inconclusive = 0

    def get_kq(num):
        sub_k = 0

        sub_t = num -1
        b_t = bin(sub_t)

        for sub_i in range(len(b_t) - 1, -1, -1):
            if b_t[sub_i] == '0':
                sub_k += 1
            else:
                break

        sub_q = sub_t >> sub_k
        return sub_k, sub_q

    k, q = get_kq(n)
    if k == 0:
        return 0

    for i in range(k):
        a = int(rnadom.uniform(2, n))
        if pow(a, q, n) == 1:
            inconclusive += 1
            continue

        t = 0
        for j in range(k):
            if pow(a, (2 * j * q), n) == n - 1:
                inconclusive += 1
                t = 1

        if t == 0:
            composite += 1

    if inconclusive >= 6:
        return 1





# 주어진 bit수에 해당하는 하나의 홀수를 생성한다.
# gen_bit = 생성할 홀수의 bit 수
# 홀수 = 리턴값
def __gen_number(gen_bit):
    random.seed()

    b = ''
    for i in range(gen_bit - 1):
        b += str(int(random.uniform(1, 10)) % 2)
    b += '1' # 생성된 마지막 bit에 1을 추가하여 홀수를 만들어낸다..   :)

    return int(b, 2)  # 앙 리턴띠




# 주어진 bit수에 해당하는 하나의 소수를 생성한다.
# gen_bit = 생성할 소수의 bit 수
# 소수 = 리턴값

def __gen_prime(gen_bit):
    while 1:
        p = __gen_number(gen_bit)
        if __mr(p) == 1:
            return p




#  n보다 작고 n과 서로소인 정수 e를 찾음
# , 확장된 유클리드 호제법을 이용해서 d * e/n으로 나우었을 때 나머지가 1인 정수 d를 찾는다.

# n = 정수
# e, d = 리턴값
def __get_ed(n):
    while 1:
        t = int(random.uniform(2, 1000))
        d, x, y = __ext_euclid(t, n)
        if d == 1:
            return  t, x




# 숫자를 문자열로 변환한다.
# val = 숫자
# 문자열 = 리턴값
def __value_to_string(val):
    ret = ''
    for i in range(32):
        b = val & 0xff
        val >>= 8
        ret += chr(b)

        if val == 0:
            break
        return ret




# 암호화를 쉽게 하기 위해서 만들어진 문자열을 숫자로 변환한다.
# buf = 문자열
# 숫자 = 리턴값
def __string_to_value(buf):
    plantext_ord = 0
    for i in range(len(buf)):
        plantext_ord |= ord(buf[i]) << (i * 8)

    return plantext_ord




# 정의된 함수들을 이용하여 rsa 개인키(skr), 공개키(pkr)을 생성한다.
# pu_fname = 공개키 파일 이름
# pr_fname = 개인키 파일 이름
# 키 생성 성공 여부 = return value
def creat_key(pu_fname='FoxVcKey.pkr', pr_fname='FoxVcKey.skr', debug=False): # 앙 여우띠!
    p = __gen_prime(128)  # 128bit 소수 생성!!!
    q = __gen_prime(128)

    n = p * q
    qn = (p - 1) * (q - 1)
    e, d = __get_ed(qn)

    pu = [e, n] # 공개키
    pr = [d, n] # 개인키

    # 공개키와 개인키를 base64로 구성한다!
    pu_data = base64.b64encode(marshal.dumps(pu))
    pr_data = base64.b64encode(marshal.dumps(pr))

    # 지정한 이름을 보유한 공개키와 개인키를 파일로 저장하여 만든다.
    try:
        open(pu_fname, 'wt').write(pu_data)
        open(pr_fname, 'wt').write(pr_data)
    except IOError:
        return False
    # 공개키와 개인키 생성을 성공적으로 완료하였다면...
    if debug:
        print '[*] Succesfully make FoxVcKeys : %s, %s .' % (pu_fname, pr_fname)
    return True # 참 반환! -> 커널에 알리기 위함.


# 암/복호화!!
# buf = 암/복호화 대상 버퍼
# key = rsa 키들
# 암/복호화된 결과물 = return Value.
def crypt(buf, key):
    plantect_ord = __strinf_to_value(buf)

    # 주어진 키로 암/복호화 시도!!!
    val = pow(plantect_ord, key[0], key[1])
    # 성공한다면 리턴한다!!!
    return __value_to_string(val)
